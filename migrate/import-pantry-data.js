...
// ==================== YAHAN APNE COMPONENT SE DATA COPY KARO ====================
...
const kitchenBasicsData = [
  {
    ...
    fullDesc: "Rice is a versatile grain...",
    storageTips: "Store in airtight container...",
    shelfLife: "White rice: 2-3 years",
    keyUses: ["Biryani", "Plain rice", "Fried rice"],
    types:
     [
...
const vegetablesData = [
  // ===== 1. 🌱 ROOT VEGETABLES =====
  {
    ...
    fullDesc: "Potatoes are the most consumed vegetable worldwide...",
    ...
    types: [
      { name: "Russet Potatoes", image: "https://images.unsplash.com/photo-1605197581314-d2d0b3929a73?auto=format&fit=crop&w=300", description: "Starchy, fluffy when cooked", bestFor: "Mashing, Baking, Frying", season: "All year", texture: "Fluffy" },
      { name: "Red Potatoes", image: "https://images.unsplash.com/photo-1587057706176-2f6c52f30c9b?auto=format&fit=crop&w=300", description: "Waxy, hold shape well", bestFor: "Salads, Roasting, Curries", season: "All year", texture: "Firm" },
      { name: "Yellow Potatoes", image: "https://images.unsplash.com/photo-1603398938378-e54eab446dde?auto=format&fit=crop&w=300", description: "Buttery flavor, medium starch", bestFor: "Roasting, Mashing", season: "All year", texture: "Creamy" }
    ]
  },
  {
    id: 1002,
    ...
    fullDesc: "Carrots are sweet, crunchy root vegetables packed with beta-carotene...",
    ...
    types: [
      { name: "Orange Carrots", image: "https://images.unsplash.com/photo-1587734195503-904137cec4a6?auto=format&fit=crop&w=300", description: "Most common variety", bestFor: "All purposes", season: "Winter", color: "Orange" },
      { name: "Red Carrots", image: "https://images.unsplash.com/photo-1563636619-e9143da7973b?auto=format&fit=crop&w=300", description: "Deep red, sweeter", bestFor: "Gajar ka Halwa, Juicing", season: "Winter", color: "Red" }
    ]
  },
  {
    id: 1003,
...
const cleanString = (str) => {
  ...
  return str.replace(/[^\w\s\u0600-\u06FF\u0900-\u097F]/g, ' ').trim();
};
...
const importData = async () => {
  try {
    ...
    if (shouldClear) {
      await BeginnersGuide.deleteMany({ 
        category: { $in: ['pantry-basics', 'spices', 'staples', 'vegetables'] } 
      });
      console.log('🗑️ Cleared existing pantry data');
    }

    // Define categories
    const categories = [
      { name: 'pantry-basics', data: kitchenBasicsData, count: kitchenBasicsData.length },
      { name: 'spices', data: spicesData, count: spicesData.length },
      { name: 'staples', data: staplesData, count: staplesData.length },
      { name: 'vegetables', data: vegetablesData, count: vegetablesData.length }
    ];

    // Import each category
    for (const category of categories) {
      console.log(`\n📦 Importing ${category.name} (${category.count} items)...`);
      ...
      for (const item of category.data) {
        try {
          ...
          const existing = await BeginnersGuide.findOne({ 
            title: item.name, 
          ...
          // Create new guide
          const guide = new BeginnersGuide({
            title: item.name,
            content: JSON.stringify(item), // Store COMPLETE object as string
            category: category.name,
            image: item.image || '',
            video: '',
            createdBy: admin._id
      ...
      console.log(`📊 ${category.name}: ${successCount} imported, ${errorCount} failed`);
    }

    // Verify import
    console.log('\n🔍 Verifying import...');
    for (const category of categories) {
      const count = await BeginnersGuide.countDocuments({ category: category.name });
      console.log(`  📁 ${category.name}: ${count} documents`);
    }
