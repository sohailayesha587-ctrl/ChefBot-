const dns = require('dns');
dns.setServers(['8.8.8.8', '1.1.1.1']);
const mongoose = require('mongoose');
const Recipe = require('../models/Recipe');
require('dotenv').config({ path: '../.env' });
const recipesToMigrate =[
  
  {
    title: "Onion Raita",
    image: "onion-raita.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "2 cups plain yogurt",
      "1 large onion, finely chopped",
      "1 green chilli, finely chopped",
      "1/4 cup fresh coriander leaves, chopped",
      "1/2 teaspoon roasted cumin powder",
      "1/4 teaspoon black salt",
      "Salt to taste",
      "1/2 teaspoon sugar",
      "1 tablespoon fresh mint leaves, chopped",
      "1/4 teaspoon black pepper powder"
    ],
    stepsRaw: [
      "Take yogurt in a mixing bowl and whisk until smooth.",
      "Add finely chopped onions to the yogurt.",
      "Add chopped green chili and coriander leaves.",
      "Mix in roasted cumin powder, black salt, and regular salt.",
      "Add sugar to balance the acidity of yogurt.",
      "Add mint leaves for extra freshness.",
      "Sprinkle black pepper powder.",
      "Mix all ingredients well.",
      "Cover and put in the fridge for at least 30 minutes.",
      "Garnish with more coriander leaves before serving.",
      "Serve chilled with biryani or rice dishes."
    ],
    difficulty: "Easy",
    cookingTime: 40,
    isActive: true,
    isHalal: true,
    description: "A cool and refreshing yogurt dip with crunchy onions and aromatic spices - great with biryani.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "economy",
    costPerServing: 20,
    baseServings: 4,
    calories: 90,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["yogurt", "onion", "green chili", "coriander", "cumin", "mint", "black salt", "sugar"],
    searchKeywords: ["onion raita", "raita", "yogurt", "side dish", "cooling"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Boondi Raita",
    image: "boondi-raita.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "2 cups plain yogurt",
      "1/2 cup boondi",
      "1/4 teaspoon roasted cumin powder",
      "1/4 teaspoon red chili powder",
      "1/4 teaspoon chaat masala",
      "Salt to taste",
      "1/2 teaspoon sugar",
      "2 tablespoons fresh coriander leaves, chopped",
      "1/4 teaspoon black salt",
      "1 tablespoon water if yogurt is thick"
    ],
    stepsRaw: [
      "Whisk yogurt in a bowl until smooth and creamy.",
      "If yogurt is too thick, add 1 tablespoon water.",
      "Add salt, sugar, and black salt to yogurt.",
      "Mix in roasted cumin powder and red chili powder.",
      "Combine all spices well into the yogurt.",
      "Add boondi to the yogurt mixture.",
      "Let it soak for 10 minutes until boondi softens slightly.",
      "Add chaat masala and mix gently.",
      "Garnish with chopped coriander leaves.",
      "Put in the fridge for 15-20 minutes before serving.",
      "Serve chilled as a refreshing side dish."
    ],
    difficulty: "Easy",
    cookingTime: 35,
    isActive: true,
    isHalal: true,
    description: "A creamy yogurt raita with soft boondi pearls and aromatic spices - a classic side dish.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "gluten"],
    budget: "economy",
    costPerServing: 25,
    baseServings: 4,
    calories: 110,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["yogurt", "boondi", "cumin", "red chili", "chaat masala", "coriander"],
    searchKeywords: ["boondi raita", "raita", "yogurt", "side dish", "cooling"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Cucumber Raita",
    image: "cucumber-raita.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "2 cups plain yogurt",
      "1 large cucumber, grated or finely chopped",
      "1/2 teaspoon roasted cumin powder",
      "1/4 teaspoon black pepper powder",
      "Salt to taste",
      "1 green chili, finely chopped",
      "2 tablespoons fresh mint leaves, chopped",
      "1 tablespoon fresh coriander leaves, chopped",
      "1/4 teaspoon sugar",
      "1 tablespoon roasted peanuts, crushed"
    ],
    stepsRaw: [
      "Peel and grate or finely chop cucumber.",
      "Squeeze out extra water from cucumber to prevent watery raita.",
      "Whisk yogurt in a bowl until smooth and creamy.",
      "Add squeezed cucumber to yogurt.",
      "Mix in roasted cumin powder and black pepper.",
      "Add salt and sugar for balanced flavor.",
      "Add chopped green chili for mild spice.",
      "Add chopped mint and coriander leaves.",
      "Mix all ingredients well.",
      "Garnish with crushed peanuts for crunch.",
      "Put in the fridge for at least 30 minutes.",
      "Serve chilled as a cooling side dish."
    ],
    difficulty: "Easy",
    cookingTime: 40,
    isActive: true,
    isHalal: true,
    description: "A refreshing cucumber raita with mint and spices - great for cooling down spicy meals.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "nuts"],
    budget: "economy",
    costPerServing: 25,
    baseServings: 4,
    calories: 95,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["yogurt", "cucumber", "cumin", "mint", "coriander", "green chili", "peanuts"],
    searchKeywords: ["cucumber raita", "raita", "yogurt", "cooling", "side dish"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Mint Raita",
    image: "mint-raita.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "2 cups plain yogurt",
      "1 cup fresh mint leaves",
      "1/4 cup fresh coriander leaves",
      "1 green chili",
      "1/2 teaspoon cumin seeds",
      "1 tablespoon lemon juice",
      "Salt to taste",
      "1/4 teaspoon black salt",
      "1/2 teaspoon sugar",
      "1 tablespoon water"
    ],
    stepsRaw: [
      "Wash mint and coriander leaves well.",
      "In a blender, combine mint, coriander, green chili, and water.",
      "Blend to make a smooth green paste.",
      "Whisk yogurt in a bowl until smooth.",
      "Add the mint-coriander paste to yogurt.",
      "Mix in lemon juice, salt, and black salt.",
      "Add sugar to balance the flavors.",
      "Dry roast cumin seeds until fragrant, then crush them.",
      "Add crushed cumin to the raita.",
      "Mix all ingredients well.",
      "Put in the fridge for at least 1 hour for flavors to develop.",
      "Garnish with mint leaves before serving.",
      "Serve chilled with spicy dishes."
    ],
    difficulty: "Easy",
    cookingTime: 70,
    isActive: true,
    isHalal: true,
    description: "A bright green mint raita that's refreshing and cooling - great with spicy biryani.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "economy",
    costPerServing: 25,
    baseServings: 4,
    calories: 85,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["yogurt", "mint", "coriander", "green chili", "cumin", "lemon"],
    searchKeywords: ["mint raita", "raita", "green raita", "yogurt", "cooling"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Potato Salad",
    image: "potatto-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "4 medium potatoes, boiled and cubed",
      "1/2 cup mayonnaise",
      "1/4 cup sour cream or thick yogurt",
      "1 tablespoon Dijon mustard",
      "1/2 cup celery, finely chopped",
      "1/4 cup red onion, finely chopped",
      "2 tablespoons fresh dill, chopped",
      "Salt to taste",
      "1/4 teaspoon black pepper",
      "2 tablespoons pickle juice or lemon juice",
      "2 hard-boiled eggs, chopped",
      "2 tablespoons fresh chives, chopped"
    ],
    stepsRaw: [
      "Boil potatoes until tender but firm, then cube them.",
      "In a large bowl, mix mayonnaise, sour cream, and Dijon mustard.",
      "Add pickle juice or lemon juice to the dressing.",
      "Season with salt and black pepper.",
      "Add chopped celery and red onion.",
      "Mix in chopped dill and chives.",
      "Gently fold in the potato cubes.",
      "Add chopped hard-boiled eggs.",
      "Mix carefully to coat all potatoes with dressing.",
      "Cover and put in the fridge for at least 2 hours.",
      "Before serving, check seasoning and adjust if needed.",
      "Garnish with more fresh herbs.",
      "Serve chilled."
    ],
    difficulty: "Medium",
    cookingTime: 150,
    isActive: true,
    isHalal: true,
    description: "A creamy and classic potato salad with dill, eggs, and a tangy mustard dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs"],
    budget: "economy",
    costPerServing: 45,
    baseServings: 4,
    calories: 280,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["potato", "mayonnaise", "sour cream", "mustard", "celery", "onion", "dill", "eggs", "chives"],
    searchKeywords: ["potato salad", "salad", "creamy", "side dish", "picnic"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Mixed Vegetable Salad",
    image: "mix-veg-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "1 cup cucumber, diced",
      "1 cup tomatoes, diced",
      "1 cup bell peppers, mixed colors, diced",
      "1/2 cup carrots, grated or cut into thin strips",
      "1/2 cup cabbage, shredded",
      "1/4 cup red onion, thinly sliced",
      "1/4 cup sweet corn, boiled",
      "2 tablespoons olive oil",
      "1 tablespoon lemon juice",
      "1 teaspoon honey or sugar",
      "Salt to taste",
      "1/4 teaspoon black pepper",
      "1/4 teaspoon dried oregano",
      "2 tablespoons fresh parsley, chopped"
    ],
    stepsRaw: [
      "Wash and prepare all vegetables as directed.",
      "In a large bowl, combine all vegetables.",
      "In a small bowl, prepare dressing: whisk olive oil, lemon juice, and honey.",
      "Add salt, black pepper, and oregano to dressing.",
      "Pour dressing over vegetables.",
      "Toss gently to coat all vegetables evenly.",
      "Add chopped parsley and mix.",
      "Let salad sit for 10-15 minutes for flavors to develop.",
      "Taste and adjust seasoning if needed.",
      "Serve fresh as a healthy side dish.",
      "Can be stored in the fridge for up to 2 hours."
    ],
    difficulty: "Easy",
    cookingTime: 20,
    isActive: true,
    isHalal: true,
    description: "A colorful and crunchy mixed vegetable salad with a light honey-oregano dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["none"],
    budget: "economy",
    costPerServing: 30,
    baseServings: 4,
    calories: 100,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["cucumber", "tomato", "bell pepper", "carrot", "cabbage", "onion", "corn", "olive oil", "lemon", "oregano"],
    searchKeywords: ["mixed vegetable salad", "salad", "healthy", "fresh", "side dish"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Fruit Chaat",
    image: "fruit-chaat.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "1 apple, diced",
      "1 banana, sliced",
      "1 orange, peeled and segmented",
      "1 cup pineapple chunks",
      "1 cup pomegranate seeds",
      "1 cup grapes, halved",
      "1/2 cup papaya, diced",
      "1/2 cup mango, diced (if in season)",
      "1 teaspoon chaat masala",
      "1/2 teaspoon black salt",
      "1/2 teaspoon roasted cumin powder",
      "1 tablespoon lemon juice",
      "2 tablespoons fresh mint leaves, chopped",
      "2 tablespoons fresh coriander leaves, chopped"
    ],
    stepsRaw: [
      "Wash and prepare all fruits as directed.",
      "In a large bowl, combine all fruits gently.",
      "Sprinkle chaat masala over fruits.",
      "Add black salt and roasted cumin powder.",
      "Drizzle lemon juice over the fruits.",
      "Gently toss to coat all fruits with spices.",
      "Add chopped mint and coriander leaves.",
      "Mix gently to avoid crushing delicate fruits.",
      "Let it sit for 5 minutes for flavors to blend.",
      "Serve immediately or chill for 15 minutes.",
      "Garnish with more mint leaves before serving.",
      "Best eaten fresh."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A tangy and spiced mixed fruit salad that's refreshing and full of flavor.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["none"],
    budget: "economy",
    costPerServing: 50,
    baseServings: 4,
    calories: 120,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["apple", "banana", "orange", "pineapple", "pomegranate", "grapes", "chaat masala", "cumin", "lemon", "mint"],
    searchKeywords: ["fruit chaat", "fruit salad", "chaat", "snack", "fresh"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Chana Chaat",
    image: "channa-chaat.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "2 cups boiled chickpeas",
      "1 medium potato, boiled and diced",
      "1 medium onion, finely chopped",
      "1 medium tomato, finely chopped",
      "1 green chili, finely chopped",
      "1/4 cup fresh coriander leaves, chopped",
      "2 tablespoons tamarind chutney",
      "2 tablespoons mint-coriander chutney",
      "1 teaspoon chaat masala",
      "1/2 teaspoon roasted cumin powder",
      "1/2 teaspoon red chili powder",
      "Salt to taste",
      "1 tablespoon lemon juice",
      "Sev for garnish",
      "Pomegranate seeds for garnish"
    ],
    stepsRaw: [
      "Boil chickpeas until tender, drain and cool.",
      "Boil potato, peel and dice it.",
      "In a large bowl, mix chickpeas and potatoes.",
      "Add chopped onion, tomato, and green chili.",
      "Add both tamarind and mint-coriander chutneys.",
      "Sprinkle chaat masala, cumin powder, and red chili powder.",
      "Add salt and lemon juice.",
      "Mix all ingredients gently but well.",
      "Garnish with fresh coriander leaves.",
      "Top with sev just before serving.",
      "Add pomegranate seeds for sweetness.",
      "Serve immediately for best taste."
    ],
    difficulty: "Easy",
    cookingTime: 25,
    isActive: true,
    isHalal: true,
    description: "A tangy and spicy chickpea chaat loaded with chutneys and fresh vegetables - a street food favorite.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["none"],
    budget: "economy",
    costPerServing: 45,
    baseServings: 4,
    calories: 180,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["chickpeas", "potato", "onion", "tomato", "green chili", "tamarind", "mint", "chaat masala", "cumin", "sev"],
    searchKeywords: ["chana chaat", "chaat", "chickpeas", "snack", "street food"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Aloo Chaat",
    image: "aloo-chaat.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "3 medium potatoes, boiled and cubed",
      "1 medium onion, finely chopped",
      "1 green chili, finely chopped",
      "2 tablespoons tamarind chutney",
      "2 tablespoons mint-coriander chutney",
      "1/4 cup fresh coriander leaves, chopped",
      "1 teaspoon chaat masala",
      "1/2 teaspoon roasted cumin powder",
      "1/2 teaspoon red chili powder",
      "Salt to taste",
      "1 tablespoon lemon juice",
      "Sev for garnish",
      "1 tablespoon pomegranate seeds",
      "1 tablespoon nylon sev"
    ],
    stepsRaw: [
      "Boil potatoes until tender but firm, cube them.",
      "Heat 1 tablespoon oil in a pan, shallow fry potatoes until golden.",
      "In a bowl, combine fried potatoes with onions and green chili.",
      "Add tamarind chutney and mint-coriander chutney.",
      "Sprinkle chaat masala, cumin powder, and red chili powder.",
      "Add salt and lemon juice.",
      "Mix gently to coat potatoes with spices.",
      "Garnish with fresh coriander leaves.",
      "Top with sev and pomegranate seeds.",
      "Serve immediately while potatoes are still warm.",
      "Can be served as snack or appetizer."
    ],
    difficulty: "Easy",
    cookingTime: 25,
    isActive: true,
    isHalal: true,
    description: "Crispy spiced potatoes tossed with tangy chutneys and crunchy sev - a tasty chaat.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["none"],
    budget: "economy",
    costPerServing: 35,
    baseServings: 4,
    calories: 160,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["potato", "onion", "green chili", "tamarind", "mint", "chaat masala", "cumin", "sev", "pomegranate"],
    searchKeywords: ["aloo chaat", "chaat", "potato", "snack", "street food"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Papri Chaat",
    image: "papri-chaat.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "20-25 papdis",
      "1 cup boiled chickpeas",
      "1 cup boiled potatoes, diced",
      "1/2 cup plain yogurt, whisked",
      "2 tablespoons tamarind chutney",
      "2 tablespoons mint-coriander chutney",
      "1 teaspoon chaat masala",
      "1/2 teaspoon roasted cumin powder",
      "1/2 teaspoon red chili powder",
      "Salt to taste",
      "1/4 cup nylon sev",
      "2 tablespoons pomegranate seeds",
      "2 tablespoons fresh coriander leaves, chopped",
      "1 tablespoon roasted cumin seeds, crushed"
    ],
    stepsRaw: [
      "Arrange papdis on a serving plate.",
      "Spread boiled chickpeas over papdis.",
      "Add boiled potato cubes over chickpeas.",
      "Drizzle whisked yogurt over everything.",
      "Add tamarind chutney and mint-coriander chutney.",
      "Sprinkle chaat masala, cumin powder, and red chili powder.",
      "Add salt to taste.",
      "Garnish generously with nylon sev.",
      "Sprinkle pomegranate seeds over sev.",
      "Add chopped coriander leaves.",
      "Sprinkle crushed cumin seeds.",
      "Serve immediately before papdis get soggy.",
      "Best enjoyed fresh."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A popular street food chaat with crispy papdis, chickpeas, yogurt, and tangy chutneys.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "gluten"],
    budget: "economy",
    costPerServing: 50,
    baseServings: 4,
    calories: 200,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["papdi", "chickpeas", "potato", "yogurt", "tamarind", "mint", "chaat masala", "sev", "pomegranate"],
    searchKeywords: ["papri chaat", "chaat", "snack", "street food", "crispy"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Dahi Bhalla",
    image: "dahi-bhallay.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "1 cup urad dal, soaked 4-6 hours",
      "2 cups plain yogurt, whisked",
      "1/4 cup tamarind chutney",
      "2 tablespoons mint-coriander chutney",
      "1 teaspoon chaat masala",
      "1/2 teaspoon roasted cumin powder",
      "1/2 teaspoon red chili powder",
      "Salt to taste",
      "1/4 teaspoon baking soda",
      "Oil for frying",
      "Water for soaking",
      "2 tablespoons pomegranate seeds",
      "2 tablespoons nylon sev",
      "2 tablespoons fresh coriander leaves, chopped"
    ],
    stepsRaw: [
      "Grind soaked urad dal to smooth batter, add salt and baking soda.",
      "Heat oil, drop spoonfuls of batter, fry until golden brown (bhallas).",
      "Soak fried bhallas in warm water for 30 minutes.",
      "Gently squeeze out water from soaked bhallas.",
      "Arrange bhallas on serving plate.",
      "Pour whisked yogurt over bhallas.",
      "Drizzle tamarind and mint-coriander chutneys.",
      "Sprinkle chaat masala, cumin powder, and red chili powder.",
      "Garnish with pomegranate seeds and nylon sev.",
      "Add chopped coriander leaves.",
      "Put in the fridge for 30 minutes before serving.",
      "Serve chilled as appetizer or snack."
    ],
    difficulty: "Hard",
    cookingTime: 90,
    isActive: true,
    isHalal: true,
    description: "Soft lentil dumplings soaked in creamy yogurt and topped with chutneys - a classic chaat.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "economy",
    costPerServing: 50,
    baseServings: 4,
    calories: 220,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["urad dal", "yogurt", "tamarind", "mint", "chaat masala", "pomegranate", "sev"],
    searchKeywords: ["dahi bhalla", "chaat", "yogurt", "snack", "street food"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Bhel Puri",
    image: "bhel-puri.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "2 cups puffed rice",
      "1/2 cup sev",
      "1/2 cup boiled potatoes, diced",
      "1/2 cup boiled chickpeas",
      "1/4 cup raw onions, finely chopped",
      "1/4 cup tomatoes, finely chopped",
      "2 tablespoons tamarind chutney",
      "2 tablespoons mint-coriander chutney",
      "1 tablespoon lemon juice",
      "1 teaspoon chaat masala",
      "1/2 teaspoon roasted cumin powder",
      "1/4 teaspoon black salt",
      "Salt to taste",
      "2 tablespoons fresh coriander leaves, chopped",
      "1 green chili, finely chopped"
    ],
    stepsRaw: [
      "Dry roast puffed rice in a pan for 2-3 minutes, let cool.",
      "In a large bowl, combine puffed rice and sev.",
      "Add boiled potatoes and chickpeas.",
      "Add chopped onions and tomatoes.",
      "Add green chili for extra spice.",
      "Add tamarind and mint-coriander chutneys.",
      "Mix in lemon juice, chaat masala, cumin powder, and black salt.",
      "Add regular salt to taste.",
      "Mix all ingredients gently but quickly.",
      "Add chopped coriander leaves.",
      "Serve immediately in bowls.",
      "Can be garnished with extra sev on top.",
      "Best enjoyed fresh and crispy."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A crunchy and tangy puffed rice snack with potatoes, chickpeas, and chutneys.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["none"],
    budget: "economy",
    costPerServing: 35,
    baseServings: 4,
    calories: 150,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["puffed rice", "sev", "potato", "chickpeas", "onion", "tomato", "tamarind", "mint", "chaat masala"],
    searchKeywords: ["bhel puri", "chaat", "snack", "street food", "crunchy"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Sev Puri",
    image: "sev-puri.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "20-25 puris",
      "2 medium potatoes, boiled and mashed",
      "1/2 cup boiled chickpeas, mashed slightly",
      "1/4 cup onions, finely chopped",
      "1/4 cup tomatoes, finely chopped",
      "2 tablespoons tamarind chutney",
      "2 tablespoons mint-coriander chutney",
      "1/4 cup nylon sev",
      "1 teaspoon chaat masala",
      "1/2 teaspoon roasted cumin powder",
      "1/2 teaspoon red chili powder",
      "Salt to taste",
      "2 tablespoons fresh coriander leaves, chopped",
      "1 tablespoon pomegranate seeds"
    ],
    stepsRaw: [
      "Arrange puris on a serving plate.",
      "Spread mashed potatoes on each puri.",
      "Add mashed chickpeas over potatoes.",
      "Sprinkle chopped onions and tomatoes.",
      "Drizzle tamarind and mint-coriander chutneys.",
      "Sprinkle chaat masala, cumin powder, and red chili powder.",
      "Add salt to taste.",
      "Generously top with nylon sev.",
      "Garnish with chopped coriander leaves.",
      "Add pomegranate seeds for sweetness.",
      "Serve immediately before puris get soggy.",
      "Best enjoyed as fresh snack."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "Crispy puris topped with spiced potatoes, chickpeas, chutneys, and crunchy sev.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "gluten"],
    budget: "economy",
    costPerServing: 45,
    baseServings: 4,
    calories: 180,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["puris", "potato", "chickpeas", "onion", "tomato", "tamarind", "mint", "sev", "chaat masala"],
    searchKeywords: ["sev puri", "chaat", "snack", "street food", "crispy"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Pakora Salad",
    image: "pakora-salad.jpg",
    cuisine: "Pakistani",
    ingredientsRaw: [
      "1 cup besan (gram flour)",
      "1 cup mixed vegetables (onion, spinach, potato)",
      "1 green chili, chopped",
      "1/2 teaspoon carom seeds",
      "1/2 teaspoon red chili powder",
      "Salt to taste",
      "Water as needed",
      "Oil for frying",
      "1 cup yogurt, whisked",
      "1/2 teaspoon roasted cumin powder",
      "1/4 teaspoon red chili powder",
      "2 tablespoons fresh coriander leaves, chopped",
      "1 tablespoon mint leaves, chopped",
      "1 tablespoon tamarind chutney"
    ],
    stepsRaw: [
      "Mix besan with spices and water to make thick batter.",
      "Add chopped vegetables to batter, mix well.",
      "Heat oil, drop spoonfuls of batter, fry until golden brown.",
      "Drain pakoras on paper towel, let cool slightly.",
      "Whisk yogurt with spices and herbs.",
      "Spread yogurt mixture on serving plate.",
      "Arrange pakoras over yogurt.",
      "Drizzle tamarind chutney over pakoras.",
      "Sprinkle more cumin powder and red chili powder.",
      "Garnish with fresh coriander leaves.",
      "Serve immediately while pakoras are still crisp."
    ],
    difficulty: "Medium",
    cookingTime: 30,
    isActive: true,
    isHalal: true,
    description: "Crispy vegetable pakoras served over spiced yogurt with tamarind chutney.",
    category: "Snacks",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "economy",
    costPerServing: 40,
    baseServings: 4,
    calories: 220,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["besan", "mixed vegetables", "carom seeds", "yogurt", "tamarind", "cumin"],
    searchKeywords: ["pakora salad", "salad", "snack", "crispy", "yogurt"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Greek Salad",
    image: "greek-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "2 large tomatoes, cut into wedges",
      "1 cucumber, sliced",
      "1 green bell pepper, sliced",
      "1 red onion, thinly sliced",
      "1 cup Kalamata olives",
      "200g feta cheese, cubed",
      "3 tablespoons extra virgin olive oil",
      "1 tablespoon red wine vinegar",
      "1 teaspoon dried oregano",
      "Salt to taste",
      "Freshly ground black pepper",
      "Fresh oregano leaves for garnish"
    ],
    stepsRaw: [
      "Wash and prepare all vegetables.",
      "In a large bowl, combine tomatoes, cucumber, bell pepper, and onion.",
      "Add Kalamata olives to the vegetables.",
      "Add cubed feta cheese.",
      "In a small bowl, whisk together olive oil and red wine vinegar.",
      "Add dried oregano, salt, and black pepper to dressing.",
      "Pour dressing over salad ingredients.",
      "Gently toss to combine all ingredients.",
      "Let salad sit for 15 minutes for flavors to blend.",
      "Garnish with fresh oregano leaves before serving.",
      "Serve at room temperature for best flavor.",
      "Can be put in the fridge for up to 2 hours."
    ],
    difficulty: "Easy",
    cookingTime: 20,
    isActive: true,
    isHalal: true,
    description: "A classic Mediterranean salad with fresh vegetables, feta cheese, and Kalamata olives.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "economy",
    costPerServing: 70,
    baseServings: 4,
    calories: 250,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["tomato", "cucumber", "bell pepper", "onion", "olives", "feta", "olive oil", "vinegar", "oregano"],
    searchKeywords: ["greek salad", "salad", "mediterranean", "fresh", "feta"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Caesar Salad",
    image: "ceaser-salad.jpg",
    cuisine: "Italian",
    ingredientsRaw: [
      "1 large head romaine lettuce, washed and torn",
      "1/2 cup Parmesan cheese, shaved",
      "1 cup croutons",
      "1/2 cup mayonnaise",
      "2 tablespoons lemon juice",
      "2 cloves garlic, minced",
      "1 teaspoon Dijon mustard",
      "1 teaspoon Worcestershire sauce",
      "1/4 cup grated Parmesan cheese",
      "Salt to taste",
      "Freshly ground black pepper",
      "2 tablespoons olive oil"
    ],
    stepsRaw: [
      "In a bowl, whisk mayonnaise, lemon juice, and garlic.",
      "Add Dijon mustard and Worcestershire sauce.",
      "Mix in grated Parmesan, salt, and pepper.",
      "Slowly whisk in olive oil until smooth.",
      "In a large bowl, combine romaine lettuce.",
      "Add croutons and Parmesan shavings.",
      "Pour dressing over salad just before serving.",
      "Toss gently to coat lettuce with dressing.",
      "Serve immediately on chilled plates.",
      "Garnish with extra Parmesan cheese.",
      "Best eaten fresh."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A classic Caesar salad with crisp romaine, creamy dressing, crunchy croutons, and Parmesan.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs", "gluten"],
    budget: "economy",
    costPerServing: 60,
    baseServings: 4,
    calories: 320,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["romaine", "parmesan", "croutons", "mayonnaise", "lemon", "garlic", "mustard", "worcestershire"],
    searchKeywords: ["caesar salad", "salad", "classic", "creamy", "romaine"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Waldorf Salad",
    image: "waldorf-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "2 large apples, diced",
      "1 cup celery, thinly sliced",
      "1/2 cup walnuts, toasted and chopped",
      "1/2 cup seedless grapes, halved",
      "1/2 cup mayonnaise",
      "1/4 cup Greek yogurt or sour cream",
      "1 tablespoon lemon juice",
      "1 teaspoon honey or sugar",
      "Salt to taste",
      "Freshly ground black pepper",
      "Lettuce leaves for serving",
      "Optional: 1/2 cup raisins or dried cranberries"
    ],
    stepsRaw: [
      "Core and dice apples, toss with lemon juice to prevent browning.",
      "Thinly slice celery stalks.",
      "Toast walnuts in dry pan until fragrant, then chop.",
      "Halve seedless grapes.",
      "In a large bowl, combine apples, celery, walnuts, and grapes.",
      "Add raisins or cranberries if using.",
      "In a small bowl, whisk mayonnaise and Greek yogurt.",
      "Add honey, salt, and pepper to dressing.",
      "Pour dressing over apple mixture.",
      "Gently fold to combine all ingredients.",
      "Chill in the fridge for at least 30 minutes.",
      "Serve on bed of lettuce leaves.",
      "Garnish with extra walnuts before serving."
    ],
    difficulty: "Easy",
    cookingTime: 45,
    isActive: true,
    isHalal: true,
    description: "A classic Waldorf salad with crisp apples, celery, walnuts, and grapes in a creamy dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs", "nuts"],
    budget: "economy",
    costPerServing: 60,
    baseServings: 4,
    calories: 280,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["apple", "celery", "walnuts", "grapes", "mayonnaise", "yogurt", "lemon", "honey"],
    searchKeywords: ["waldorf salad", "salad", "apple", "creamy", "classic"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Caprese Salad",
    image: "caprese-salad.jpg",
    cuisine: "Italian",
    ingredientsRaw: [
      "3-4 large ripe tomatoes, sliced",
      "250g fresh mozzarella cheese, sliced",
      "1 bunch fresh basil leaves",
      "3 tablespoons extra virgin olive oil",
      "1 tablespoon balsamic vinegar",
      "Salt to taste",
      "Freshly ground black pepper",
      "Optional: balsamic glaze for drizzling"
    ],
    stepsRaw: [
      "Slice tomatoes and mozzarella to similar thickness.",
      "Arrange tomato and mozzarella slices alternately on plate.",
      "Tuck fresh basil leaves between slices.",
      "In a small bowl, whisk olive oil and balsamic vinegar.",
      "Add salt and pepper to dressing.",
      "Drizzle dressing over arranged salad.",
      "Season with more salt and pepper to taste.",
      "Let salad sit for 10 minutes for flavors to develop.",
      "Drizzle with balsamic glaze if using.",
      "Garnish with whole basil leaves.",
      "Serve at room temperature for best flavor.",
      "Can be served as appetizer or side dish."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A simple and elegant Italian salad with fresh tomatoes, mozzarella, and basil.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "economy",
    costPerServing: 70,
    baseServings: 4,
    calories: 220,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["tomato", "mozzarella", "basil", "olive oil", "balsamic"],
    searchKeywords: ["caprese salad", "salad", "italian", "fresh", "tomato"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Cobb Salad",
    image: "cobb-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "1 head romaine lettuce, chopped",
      "2 hard-boiled eggs, chopped",
      "2 cooked chicken breasts, diced",
      "6 strips bacon, cooked and crumbled",
      "1 avocado, diced",
      "2 tomatoes, diced",
      "1/2 cup blue cheese, crumbled",
      "1/4 cup red onion, thinly sliced",
      "1/2 cup olive oil",
      "1/4 cup red wine vinegar",
      "1 tablespoon Dijon mustard",
      "1 clove garlic, minced",
      "1 teaspoon honey",
      "Salt and pepper to taste"
    ],
    stepsRaw: [
      "Prepare all ingredients as directed.",
      "Arrange chopped lettuce on large platter.",
      "Arrange other ingredients in rows over lettuce.",
      "For dressing: whisk olive oil, vinegar, and Dijon mustard.",
      "Add garlic, honey, salt, and pepper to dressing.",
      "Drizzle dressing over salad just before serving.",
      "Alternatively, serve dressing on side.",
      "Toss salad at table or serve as arranged.",
      "Serve immediately for best texture."
    ],
    difficulty: "Medium",
    cookingTime: 25,
    isActive: true,
    isHalal: true,
    description: "A hearty chopped salad with eggs, chicken, bacon, avocado, and blue cheese.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Non-Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs"],
    budget: "standard",
    costPerServing: 100,
    baseServings: 4,
    calories: 450,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["romaine", "eggs", "chicken", "bacon", "avocado", "tomato", "blue cheese", "onion", "mustard"],
    searchKeywords: ["cobb salad", "salad", "hearty", "chopped", "protein"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Nicoise Salad",
    image: "spring-rolls.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "200g fresh tuna steak or 2 cans tuna in olive oil",
      "4 small potatoes, boiled and quartered",
      "200g green beans, trimmed",
      "4 hard-boiled eggs, quartered",
      "2 tomatoes, cut into wedges",
      "1/2 cup black olives",
      "1/4 cup red onion, thinly sliced",
      "1/4 cup olive oil",
      "2 tablespoons lemon juice",
      "1 tablespoon Dijon mustard",
      "1 clove garlic, minced",
      "1 teaspoon dried oregano",
      "Salt and pepper to taste"
    ],
    stepsRaw: [
      "Grill or sear tuna steak until cooked to your liking, slice.",
      "Boil potatoes until tender, quarter when cooled.",
      "Blanch green beans in boiling water for 3 minutes, then ice bath.",
      "Arrange all ingredients attractively on platter.",
      "In a small bowl, whisk olive oil, lemon juice, and mustard.",
      "Add garlic, oregano, salt, and pepper to dressing.",
      "Drizzle dressing over salad just before serving.",
      "Serve at room temperature."
    ],
    difficulty: "Medium",
    cookingTime: 30,
    isActive: true,
    isHalal: true,
    description: "A classic French composed salad with tuna, potatoes, green beans, and eggs.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Non-Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["eggs", "fish"],
    budget: "standard",
    costPerServing: 120,
    baseServings: 4,
    calories: 380,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["tuna", "potato", "green beans", "eggs", "tomato", "olives", "onion", "mustard", "oregano"],
    searchKeywords: ["nicoise salad", "salad", "french", "tuna", "composed"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Coleslaw",
    image: "coleslaw.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "4 cups cabbage, finely shredded",
      "2 carrots, grated",
      "1/2 cup mayonnaise",
      "2 tablespoons sour cream or yogurt",
      "1 tablespoon apple cider vinegar",
      "1 tablespoon sugar",
      "1 teaspoon Dijon mustard",
      "1/2 teaspoon celery seeds",
      "Salt to taste",
      "Freshly ground black pepper",
      "2 tablespoons fresh parsley, chopped",
      "1/4 cup red onion, finely chopped"
    ],
    stepsRaw: [
      "Finely shred cabbage using a sharp knife or mandoline.",
      "Grate carrots using large holes of grater.",
      "In a large bowl, combine cabbage and carrots.",
      "In a separate bowl, whisk mayonnaise and sour cream.",
      "Add apple cider vinegar, sugar, and Dijon mustard.",
      "Mix in celery seeds if using, salt, and pepper.",
      "Pour dressing over cabbage mixture.",
      "Add chopped parsley and red onion if using.",
      "Toss well to coat all vegetables.",
      "Cover and put in the fridge for at least 2 hours.",
      "Stir before serving to redistribute dressing.",
      "Serve chilled."
    ],
    difficulty: "Easy",
    cookingTime: 135,
    isActive: true,
    isHalal: true,
    description: "A creamy and tangy coleslaw with shredded cabbage and carrots - great for BBQ.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs"],
    budget: "economy",
    costPerServing: 30,
    baseServings: 4,
    calories: 180,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["cabbage", "carrot", "mayonnaise", "sour cream", "vinegar", "sugar", "mustard", "celery seeds"],
    searchKeywords: ["coleslaw", "salad", "creamy", "bbq side", "cabbage"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Garden Salad",
    image: "garden-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "4 cups mixed lettuce greens",
      "1 cucumber, sliced",
      "2 tomatoes, cut into wedges",
      "1 bell pepper, any color, sliced",
      "1/2 cup radishes, sliced",
      "1/4 cup red onion, thinly sliced",
      "1 carrot, shredded",
      "3 tablespoons olive oil",
      "1 tablespoon red wine vinegar",
      "1 teaspoon Dijon mustard",
      "1/2 teaspoon honey",
      "1 clove garlic, minced",
      "Salt and pepper to taste",
      "1 teaspoon dried oregano"
    ],
    stepsRaw: [
      "Wash and dry lettuce greens well.",
      "Prepare all vegetables as directed.",
      "In a large salad bowl, combine all vegetables.",
      "In a small jar, combine all dressing ingredients.",
      "Shake dressing well until well combined.",
      "Pour dressing over salad just before serving.",
      "Toss gently to coat all ingredients.",
      "Serve immediately for best texture.",
      "Can customize with other vegetables."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A fresh and simple garden salad with mixed greens and a light vinaigrette.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["none"],
    budget: "economy",
    costPerServing: 35,
    baseServings: 4,
    calories: 120,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["lettuce", "cucumber", "tomato", "bell pepper", "radish", "onion", "carrot", "olive oil", "vinegar", "mustard", "oregano"],
    searchKeywords: ["garden salad", "salad", "fresh", "simple", "green"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Russian Salad",
    image: "russian-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "2 potatoes, boiled and diced",
      "2 carrots, boiled and diced",
      "1 cup green peas, boiled",
      "1 cup French beans, boiled and chopped",
      "1/2 cup sweet corn, boiled",
      "1 apple, peeled and diced",
      "1/2 cup mayonnaise",
      "1/4 cup sour cream or yogurt",
      "1 tablespoon Dijon mustard",
      "1 tablespoon lemon juice",
      "Salt to taste",
      "Freshly ground black pepper",
      "1/4 cup fresh dill, chopped"
    ],
    stepsRaw: [
      "Boil all vegetables separately until tender but firm.",
      "Allow all vegetables to cool completely.",
      "Dice potatoes, carrots, and other vegetables uniformly.",
      "In a large bowl, combine all vegetables and apple.",
      "In a separate bowl, whisk mayonnaise and sour cream.",
      "Add Dijon mustard, lemon juice, salt, and pepper.",
      "Pour dressing over vegetable mixture.",
      "Gently fold to combine all ingredients.",
      "Add chopped dill and mix.",
      "Cover and put in the fridge for at least 4 hours.",
      "Stir before serving.",
      "Garnish with more dill before serving."
    ],
    difficulty: "Medium",
    cookingTime: 270,
    isActive: true,
    isHalal: true,
    description: "A creamy Russian salad with boiled vegetables, apples, and a tangy mayonnaise dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs"],
    budget: "economy",
    costPerServing: 45,
    baseServings: 4,
    calories: 250,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["potato", "carrot", "peas", "beans", "corn", "apple", "mayonnaise", "sour cream", "dill"],
    searchKeywords: ["russian salad", "salad", "creamy", "vegetable", "classic"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Continental Potato Salad",
    image: "continental-potato-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "6 medium potatoes, boiled and cubed",
      "4 hard-boiled eggs, chopped",
      "1/2 cup celery, finely chopped",
      "1/4 cup red onion, finely chopped",
      "1/4 cup sweet pickle relish",
      "1 cup mayonnaise",
      "2 tablespoons yellow mustard",
      "1 tablespoon apple cider vinegar",
      "1 teaspoon sugar",
      "Salt to taste",
      "Freshly ground black pepper",
      "1/4 cup fresh parsley, chopped",
      "Paprika for garnish"
    ],
    stepsRaw: [
      "Boil potatoes until tender but firm, cube when cooled.",
      "Hard boil eggs, cool, peel and chop.",
      "In a large bowl, combine potatoes, eggs, celery, and onion.",
      "In a separate bowl, whisk mayonnaise and mustard.",
      "Add pickle relish, vinegar, sugar, salt, and pepper.",
      "Pour dressing over potato mixture.",
      "Gently fold to combine all ingredients.",
      "Add chopped parsley and mix.",
      "Cover and put in the fridge for at least 4 hours.",
      "Before serving, taste and adjust seasoning.",
      "Sprinkle with paprika for garnish.",
      "Serve chilled."
    ],
    difficulty: "Medium",
    cookingTime: 270,
    isActive: true,
    isHalal: true,
    description: "A classic American-style potato salad with eggs, celery, and a tangy mustard dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs"],
    budget: "economy",
    costPerServing: 50,
    baseServings: 4,
    calories: 320,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["potato", "eggs", "celery", "onion", "pickle relish", "mayonnaise", "mustard", "paprika"],
    searchKeywords: ["potato salad", "salad", "classic", "creamy", "bbq side"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Macaroni Salad",
    image: "continental-macroni-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "2 cups elbow macaroni, cooked",
      "1/2 cup mayonnaise",
      "1/4 cup sour cream or yogurt",
      "1 tablespoon Dijon mustard",
      "1 tablespoon apple cider vinegar",
      "1 teaspoon sugar",
      "1/2 cup celery, finely chopped",
      "1/4 cup red onion, finely chopped",
      "1/2 cup sweet corn, boiled",
      "1/4 cup black olives, sliced",
      "1/4 cup fresh parsley, chopped",
      "Salt to taste",
      "Freshly ground black pepper",
      "Paprika for garnish"
    ],
    stepsRaw: [
      "Cook macaroni according to package directions, drain, rinse with cold water.",
      "Allow macaroni to cool completely.",
      "In a large bowl, whisk mayonnaise and sour cream.",
      "Add Dijon mustard, vinegar, sugar, salt, and pepper.",
      "Add cooled macaroni to dressing mixture.",
      "Mix in celery, onion, sweet corn, and olives.",
      "Add chopped parsley and mix well.",
      "Cover and put in the fridge for at least 2 hours.",
      "Stir before serving to redistribute dressing.",
      "Taste and adjust seasoning if needed.",
      "Sprinkle with paprika before serving.",
      "Serve chilled."
    ],
    difficulty: "Easy",
    cookingTime: 135,
    isActive: true,
    isHalal: true,
    description: "A creamy macaroni salad with crunchy vegetables and a tangy dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs", "gluten"],
    budget: "economy",
    costPerServing: 45,
    baseServings: 4,
    calories: 280,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["macaroni", "mayonnaise", "sour cream", "mustard", "celery", "onion", "corn", "olives"],
    searchKeywords: ["macaroni salad", "salad", "creamy", "pasta", "side dish"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Fattoush Salad",
    image: "fattoush-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "4 cups romaine lettuce, chopped",
      "2 tomatoes, diced",
      "1 cucumber, diced",
      "1 bell pepper, diced",
      "4 radishes, thinly sliced",
      "4 green onions, chopped",
      "1/2 cup fresh mint leaves, chopped",
      "1/2 cup fresh parsley, chopped",
      "2 pita breads, toasted and broken into pieces",
      "1/4 cup olive oil",
      "2 tablespoons lemon juice",
      "1 tablespoon pomegranate molasses",
      "1 clove garlic, minced",
      "1 teaspoon sumac",
      "Salt to taste",
      "Freshly ground black pepper"
    ],
    stepsRaw: [
      "Toast pita bread until crisp, break into bite-sized pieces.",
      "Wash and prepare all vegetables.",
      "In a large bowl, combine lettuce, tomatoes, cucumber, and bell pepper.",
      "Add radishes, green onions, mint, and parsley.",
      "In a small bowl, whisk all dressing ingredients together.",
      "Pour dressing over salad just before serving.",
      "Add toasted pita pieces and toss gently.",
      "Serve immediately to prevent pita from getting soggy.",
      "Can prepare vegetables ahead, add dressing last.",
      "Sumac adds the tangy flavor."
    ],
    difficulty: "Easy",
    cookingTime: 20,
    isActive: true,
    isHalal: true,
    description: "A Lebanese bread salad with fresh vegetables and tangy sumac dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["gluten"],
    budget: "economy",
    costPerServing: 50,
    baseServings: 4,
    calories: 180,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["romaine", "tomato", "cucumber", "bell pepper", "radish", "green onion", "mint", "parsley", "pita", "sumac", "pomegranate molasses"],
    searchKeywords: ["fattoush salad", "salad", "lebanese", "bread salad", "middle eastern"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Tabbouleh",
    image: "tabbouleh-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "1 cup fine bulgur wheat",
      "2 cups boiling water",
      "4 cups fresh parsley, finely chopped",
      "1/2 cup fresh mint leaves, finely chopped",
      "4 tomatoes, finely diced",
      "1 cucumber, finely diced",
      "4 green onions, finely chopped",
      "1/4 cup lemon juice",
      "1/4 cup olive oil",
      "Salt to taste",
      "Freshly ground black pepper",
      "1 teaspoon ground cumin",
      "Lettuce leaves for serving"
    ],
    stepsRaw: [
      "Place bulgur in a bowl, pour boiling water over it.",
      "Cover and let soak for 30 minutes until tender.",
      "Drain any extra water, fluff with fork.",
      "Finely chop parsley and mint leaves.",
      "Dice tomatoes and cucumber very finely.",
      "Chop green onions finely.",
      "In a large bowl, combine soaked bulgur and chopped herbs.",
      "Add tomatoes, cucumber, and green onions.",
      "In a small bowl, whisk lemon juice and olive oil.",
      "Add salt, pepper, and cumin to dressing.",
      "Pour dressing over salad mixture.",
      "Toss gently to combine all ingredients.",
      "Serve on lettuce leaves or with pita bread."
    ],
    difficulty: "Easy",
    cookingTime: 40,
    isActive: true,
    isHalal: true,
    description: "A fresh and herby Lebanese tabbouleh salad with bulgur, parsley, and mint.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["gluten"],
    budget: "economy",
    costPerServing: 45,
    baseServings: 4,
    calories: 160,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["bulgur", "parsley", "mint", "tomato", "cucumber", "green onion", "lemon", "olive oil", "cumin"],
    searchKeywords: ["tabbouleh", "salad", "lebanese", "herb", "bulgur"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
 
  {
    title: "Egg Salad",
    image: "egg-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "8 hard-boiled eggs",
      "1/2 cup mayonnaise",
      "1 tablespoon Dijon mustard",
      "1 tablespoon lemon juice",
      "1/4 cup celery, finely chopped",
      "2 tablespoons red onion, finely chopped",
      "2 tablespoons sweet pickle relish",
      "1 tablespoon fresh dill, chopped",
      "1 tablespoon fresh chives, chopped",
      "Salt to taste",
      "Freshly ground black pepper",
      "Paprika for garnish",
      "Bread or lettuce leaves for serving"
    ],
    stepsRaw: [
      "Hard boil eggs, cool completely in ice water.",
      "Peel eggs and chop coarsely.",
      "In a large bowl, combine chopped eggs.",
      "Add celery, onion, and pickle relish.",
      "In a small bowl, whisk mayonnaise and Dijon mustard.",
      "Add lemon juice, salt, and pepper to dressing.",
      "Pour dressing over egg mixture.",
      "Add chopped dill and chives.",
      "Mix gently to combine all ingredients.",
      "Cover and put in the fridge for at least 30 minutes.",
      "Serve on bread for sandwiches or with crackers.",
      "Sprinkle with paprika before serving."
    ],
    difficulty: "Easy",
    cookingTime: 45,
    isActive: true,
    isHalal: true,
    description: "A classic creamy egg salad with fresh herbs and a tangy dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs"],
    budget: "economy",
    costPerServing: 35,
    baseServings: 4,
    calories: 280,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["eggs", "mayonnaise", "mustard", "celery", "onion", "pickle relish", "dill", "chives", "paprika"],
    searchKeywords: ["egg salad", "salad", "creamy", "sandwich filling", "protein"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Beetroot Salad",
    image: "beetroot-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "4 medium beetroots, boiled or roasted",
      "2 oranges, peeled and segmented",
      "1/2 cup walnuts, toasted and chopped",
      "4 cups mixed salad greens",
      "1/4 cup red onion, thinly sliced",
      "1/2 cup goat cheese or feta cheese, crumbled",
      "1/4 cup olive oil",
      "2 tablespoons orange juice",
      "1 tablespoon balsamic vinegar",
      "1 teaspoon Dijon mustard",
      "1 teaspoon honey",
      "Salt to taste",
      "Freshly ground black pepper",
      "Fresh mint leaves for garnish"
    ],
    stepsRaw: [
      "Boil or roast beetroots until tender, peel and slice.",
      "Toast walnuts in dry pan until fragrant, then chop.",
      "Peel oranges and segment, removing membranes.",
      "Arrange salad greens on serving platter.",
      "Arrange beetroot slices and orange segments over greens.",
      "Sprinkle with toasted walnuts and sliced red onion.",
      "Crumble cheese over salad.",
      "In a small bowl, whisk all dressing ingredients together.",
      "Drizzle dressing over salad just before serving.",
      "Garnish with fresh mint leaves.",
      "Serve immediately."
    ],
    difficulty: "Medium",
    cookingTime: 60,
    isActive: true,
    isHalal: true,
    description: "A bright beetroot salad with orange segments, walnuts, and creamy goat cheese.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "nuts"],
    budget: "standard",
    costPerServing: 80,
    baseServings: 4,
    calories: 280,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["beetroot", "orange", "walnuts", "salad greens", "onion", "goat cheese", "balsamic", "mustard", "honey"],
    searchKeywords: ["beetroot salad", "salad", "roasted beet", "healthy", "goat cheese"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Avocado Salad",
    image: "avocado-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "2 ripe avocados, diced",
      "2 tomatoes, diced",
      "1 cucumber, diced",
      "1/2 red onion, thinly sliced",
      "1 jalapeño pepper, seeded and finely chopped",
      "1/4 cup fresh coriander leaves, chopped",
      "2 tablespoons lime juice",
      "2 tablespoons olive oil",
      "Salt to taste",
      "Freshly ground black pepper",
      "1 teaspoon cumin powder",
      "Optional: 1 cup corn kernels, cooked",
      "Tortilla chips for serving"
    ],
    stepsRaw: [
      "Cut avocados in half, remove pit, dice flesh.",
      "Immediately toss avocado with lime juice to prevent browning.",
      "Dice tomatoes and cucumber.",
      "Thinly slice red onion.",
      "Seed and finely chop jalapeño pepper.",
      "In a large bowl, combine all prepared vegetables.",
      "Add chopped coriander leaves.",
      "In a small bowl, whisk olive oil, remaining lime juice, cumin, salt, and pepper.",
      "Pour dressing over salad mixture.",
      "Gently toss to combine all ingredients.",
      "Add corn kernels if using.",
      "Serve immediately with tortilla chips.",
      "Best eaten fresh as avocados brown quickly."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A fresh avocado salad with tomatoes, cucumber, and a zesty lime dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["none"],
    budget: "standard",
    costPerServing: 70,
    baseServings: 4,
    calories: 220,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["avocado", "tomato", "cucumber", "onion", "jalapeño", "coriander", "lime", "cumin"],
    searchKeywords: ["avocado salad", "salad", "fresh", "healthy", "mexican"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  
  {
    title: "Broccoli Salad",
    image: "broccoli-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "4 cups broccoli florets, chopped into bite-sized pieces",
      "1/2 cup red onion, finely chopped",
      "1/2 cup dried cranberries or raisins",
      "1/2 cup sunflower seeds or pumpkin seeds",
      "1/2 cup bacon, cooked and crumbled",
      "1/2 cup cheddar cheese, shredded",
      "1 cup mayonnaise",
      "2 tablespoons apple cider vinegar",
      "2 tablespoons sugar",
      "Salt to taste",
      "Freshly ground black pepper"
    ],
    stepsRaw: [
      "Chop broccoli florets into small, bite-sized pieces.",
      "Finely chop red onion.",
      "Cook bacon until crisp, drain and crumble.",
      "In a large bowl, combine broccoli, onion, cranberries, and seeds.",
      "Add crumbled bacon and shredded cheese.",
      "In a separate bowl, whisk mayonnaise and vinegar.",
      "Add sugar, salt, and pepper to dressing.",
      "Pour dressing over broccoli mixture.",
      "Mix well to coat all ingredients.",
      "Cover and put in the fridge for at least 2 hours.",
      "Stir before serving.",
      "Flavors improve with longer refrigeration."
    ],
    difficulty: "Easy",
    cookingTime: 135,
    isActive: true,
    isHalal: true,
    description: "A crunchy broccoli salad with bacon, cranberries, and a sweet tangy dressing.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Non-Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy", "eggs"],
    budget: "standard",
    costPerServing: 80,
    baseServings: 4,
    calories: 380,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["broccoli", "onion", "cranberries", "seeds", "bacon", "cheddar", "mayonnaise", "vinegar"],
    searchKeywords: ["broccoli salad", "salad", "crunchy", "bacon", "potluck"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Feta Cheese Salad",
    image: "feta-cheese-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "4 cups mixed salad greens",
      "1 cup cherry tomatoes, halved",
      "1 cucumber, sliced",
      "1/2 red onion, thinly sliced",
      "1 bell pepper, sliced",
      "1 cup Kalamata olives",
      "200g feta cheese, cubed",
      "1/4 cup olive oil",
      "2 tablespoons red wine vinegar",
      "1 teaspoon dried oregano",
      "1 clove garlic, minced",
      "1 teaspoon Dijon mustard",
      "Salt to taste",
      "Freshly ground black pepper",
      "Fresh oregano leaves for garnish"
    ],
    stepsRaw: [
      "Wash and dry salad greens well.",
      "Prepare all vegetables as directed.",
      "Cube feta cheese.",
      "In a large salad bowl, combine greens and all vegetables.",
      "Add Kalamata olives and feta cheese cubes.",
      "In a small bowl, whisk olive oil and red wine vinegar.",
      "Add oregano, garlic, Dijon mustard, salt, and pepper.",
      "Whisk dressing until well combined.",
      "Pour dressing over salad just before serving.",
      "Toss gently to coat all ingredients.",
      "Garnish with fresh oregano leaves.",
      "Serve immediately."
    ],
    difficulty: "Easy",
    cookingTime: 15,
    isActive: true,
    isHalal: true,
    description: "A classic Greek-style salad with feta cheese, olives, and fresh vegetables.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "standard",
    costPerServing: 70,
    baseServings: 4,
    calories: 280,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["salad greens", "cherry tomatoes", "cucumber", "onion", "bell pepper", "olives", "feta", "oregano"],
    searchKeywords: ["feta cheese salad", "salad", "greek", "mediterranean", "fresh"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  },
  {
    title: "Mediterranean Salad",
    image: "mediterranean-salad.jpg",
    cuisine: "Continental",
    ingredientsRaw: [
      "2 cups cooked quinoa or couscous",
      "1 cup cherry tomatoes, halved",
      "1 cucumber, diced",
      "1 bell pepper, diced",
      "1/2 cup Kalamata olives, sliced",
      "1/2 cup feta cheese, crumbled",
      "1/4 cup red onion, finely chopped",
      "1/4 cup fresh parsley, chopped",
      "1/4 cup fresh mint leaves, chopped",
      "1/4 cup olive oil",
      "2 tablespoons lemon juice",
      "1 clove garlic, minced",
      "1 teaspoon dried oregano",
      "Salt to taste",
      "Freshly ground black pepper"
    ],
    stepsRaw: [
      "Cook quinoa or couscous according to package directions, cool.",
      "Prepare all vegetables as directed.",
      "Chop fresh herbs finely.",
      "In a large bowl, combine cooled grain and vegetables.",
      "Add olives, feta cheese, and herbs.",
      "In a small bowl, whisk all dressing ingredients together.",
      "Pour dressing over salad mixture.",
      "Toss gently to combine all ingredients.",
      "Cover and put in the fridge for at least 1 hour.",
      "Stir before serving.",
      "Can be served cold or at room temperature.",
      "Flavors improve with time."
    ],
    difficulty: "Easy",
    cookingTime: 90,
    isActive: true,
    isHalal: true,
    description: "A healthy Mediterranean grain salad with quinoa, fresh vegetables, and feta cheese.",
    category: "Lunch",
    subCategory: "salads",
    dietType: "Vegetarian",
    suitableForMeals: ["Lunch", "Dinner", "Anytime"],
    allergens: ["dairy"],
    budget: "standard",
    costPerServing: 70,
    baseServings: 4,
    calories: 280,
    ageGroup: ["adults", "kids", "seniors"],
    patientFriendly: ["general", "diabetes", "heart", "bp", "lowsalt", "lowfat"],
    pantryKeywords: ["quinoa", "cherry tomatoes", "cucumber", "bell pepper", "olives", "feta", "parsley", "mint", "oregano"],
    searchKeywords: ["mediterranean salad", "salad", "quinoa", "healthy", "grain"],
    isFeatured: false,
    averageRating: 0,
    totalRatings: 0,
    timesSuggested: 0,
    timesUsedInPlans: 0
  }
]
async function migrateRecipes() {
  try {
    console.log('Connecting to MongoDB');
    
    const mongoURI = process.env.MONGO_URI || process.env.MONGODB_URI || process.env.MONGOURI;
    if (!mongoURI) {
      throw new Error('MongoDB URI not found');
    }
    
    await mongoose.connect(mongoURI, { 
      serverSelectionTimeoutMS: 10000, 
      family: 4 
    });
    
    console.log('Connected to MongoDB!');

    let added = 0;
    let skipped = 0;

    for (let recipe of recipesToMigrate) {
      const existing = await Recipe.findOne({ title: recipe.title });
      
      if (!existing) {
        await Recipe.create(recipe);
        console.log(`Added: ${recipe.title}`);
        added++;
      } else {
        console.log(`Skipped: ${recipe.title} (already exists)`);
        skipped++;
      }
    }

    console.log('\nMigration Complete!');
    console.log(`Added: ${added}, Skipped: ${skipped}`);
    
    process.exit(0);
    
  } catch (error) {
    console.error('Error:', error);
    process.exit(1);
  }
}

migrateRecipes();